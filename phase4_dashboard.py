"""
Weather Dashboard — India Edition
A weather dashboard using OpenWeatherMap API, tuned for Indian users.
...
import zoneinfo  # Python 3.9+; fallback below
...
def to_ist(utc_ts: int) -> datetime:
    """Convert a UNIX UTC timestamp to IST datetime."""
    return datetime.fromtimestamp(utc_ts, tz=IST)

def fmt_ist(utc_ts: int, fmt: str = "%I:%M %p") -> str:
    """Format a UNIX UTC timestamp as an IST time string."""
    return to_ist(utc_ts).strftime(fmt)
...
# Indian cities first, a few international cities at the bottom
CITIES = {
...
# Season helper (for Indian meteorological seasons)
def indian_season(month: int) -> str:
    if month in (3, 4, 5):    return "🌸 Summer"
    if month in (6, 7, 8, 9): return "🌧️ Monsoon"
...
def get_forecast(city, lat, lon):
    ...
    r.raise_for_status()
    return pd.DataFrame([{
        # Convert each forecast timestamp to IST
        "dt":        to_ist(i["dt"]).replace(tzinfo=None),  # strip tz for plotting
        "temp":      i["main"]["temp"],
...
def make_temp_chart(fc, ml_val):
    d = fc.copy()
    d["date"] = pd.to_datetime(d["dt"]).dt.date
    ...
    if ml_val:
        # Tomorrow in IST
        tom = pd.Timestamp((now_ist() + timedelta(days=1)).date())
        fig.add_trace(go.Scatter(
    ...
    fig.update_layout(
        ...
        xaxis_title="Date (IST)", yaxis_title="°C",
...
def make_pressure_chart(fc):
    ...
    fig.update_layout(
        ...
        xaxis_title="Date/Time (IST)", yaxis_title="hPa",
...
# Filter out the separator entry
valid_cities = {k: v for k, v in CITIES.items() if v is not None}
city = st.sidebar.selectbox("Select City", list(valid_cities.keys()), index=0)
...
st.sidebar.markdown("---")
st.sidebar.markdown("🕐 All times shown in **IST (UTC+5:30)**")
st.sidebar.markdown(f"📅 Today: **{now_ist().strftime('%d %b %Y, %I:%M %p IST')}**")

lat, lon = valid_cities[city]
...
wind_k = wind * 3.6                          # m/s → km/h (more familiar in India)
...
# ── Times in IST ──────────────────────────────────────────────────────────────
sr_ist  = fmt_ist(current["sys"]["sunrise"])          # e.g. "05:48 AM"
ss_ist  = fmt_ist(current["sys"]["sunset"])           # e.g. "07:12 PM"
upd_ist = fmt_ist(current["dt"], "%d %b, %I:%M %p")  # e.g. "16 Apr, 02:30 PM"

season  = indian_season(now_ist().month)
ml_val  = ml_predict(current, fc)

# Daily summary for forecast cards
fc_d   = fc.copy()
fc_d["date"] = pd.to_datetime(fc_d["dt"]).dt.date
...
with col_title1:
    st.caption(f"Last updated: **{upd_ist} IST** · Source: OpenWeatherMap")
with col_title2:
...
col4.metric("🌬 Wind Speed", f"{wind_k:.0f} km/h", f"{wind:.1f} m/s")

col5, col6, col7, col8 = st.columns(4)
col5.metric("📊 Pressure", f"{pres} hPa")
col6.metric("👁 Visibility", f"{vis} km")
col7.metric("☁️ Cloud Cover", f"{clouds} %")
...
for i, (_, row) in enumerate(daily.iterrows()):
    label = "Today" if row["date"] == today_ist else pd.Timestamp(row["date"]).strftime("%a, %d %b")
...
with d1:
    st.markdown("**☀️ Sun Timings (IST)**")
    st.write(f"🌅 Sunrise: {sr_ist} IST")
    st.write(f"🌇 Sunset:  {ss_ist} IST")
...
with d3:
    ...
    st.write(f"💨 Wind: {wind_k:.1f} km/h ({wind:.1f} m/s)")
    wind_dir = current["wind"].get("deg", None)
    if wind_dir is not None:
        dirs = ["N","NNE","NE","ENE","E","ESE","SE","SSE","S","SSW","SW","WSW","W","WNW","NW","NNW"]
        arrow = dirs[round(wind_dir / 22.5) % 16]
        st.write(f"🧭 Direction: {arrow} ({wind_dir}°)")
    st.write(f"📊 Pressure: {pres} hPa")
    rain_1h = current.get("rain", {}).get("1h", 0)
    if rain_1h:
        st.write(f"🌧️ Rain (last 1h): {rain_1h:.1f} mm")

st.divider()
st.caption("Weather Dashboard · All times in IST (UTC+5:30) · Data: OpenWeatherMap API")
